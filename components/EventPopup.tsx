...
// export default function EventPopup() {
//   const [isOpen, setIsOpen] = useState(false);
//   const [activeTab, setActiveTab] = useState<'world-record' | 'volunteer' | 'event' | 'train'>('world-record');
//   const [searchTerm, setSearchTerm] = useState("");
...
//               {/* 1. TABS HEADER */}
//               <div className="flex-none flex overflow-x-auto border-b border-gray-200 scrollbar-hide">
//                 {/* TAB 0: WORLD RECORD */}
//                 <button
//                   onClick={() => setActiveTab('world-record')}
//                   className={`flex-1 min-w-[120px] py-5 text-xs md:text-sm font-bold uppercase tracking-wide transition-colors relative ${
//                     activeTab === 'world-record' ? 'text-purple-700 bg-purple-50/50' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'
//                   }`}
//                 >
//                   <span className="flex items-center justify-center gap-2">
...
//                     <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 w-full h-1 bg-purple-700" />
//                   )}
//                 </button>
...
//                     exit={{ opacity: 0, y: -10 }}
//                     transition={{ duration: 0.2 }}
//                     className="h-full flex flex-col items-center space-y-4"
//                   >
//                     <div className="w-full h-full relative min-h-[300px] md:min-h-[400px] rounded-xl overflow-hidden bg-purple-50 flex items-center justify-center border border-purple-100">
//                       <Image 
//                         src="https://res.cloudinary.com/dvd7o5nph/image/upload/v1771673388/WORLD_RECORD_POSTER.jpg_kkmzi9.jpg" 
//                         // src="/poster.jpg" 
//                         alt="World Record Attempt Poster" 
//                         fill
...
//                   {/* Footer for WORLD RECORD Tab */}
//                   {activeTab === 'world-record' && (
//                     <div className="space-y-3">
//                       <Link 
//                         href="/notices/notification" 
//                         onClick={() => setIsOpen(false)}
//                         className="w-full py-3 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition-all flex items-center justify-center gap-2 text-sm shadow-md group"
//                       >
//                         Click Here to Know More <Trophy className="w-4 h-4 group-hover:scale-110 transition-transform" />
//                       </Link>
//                     </div>
//                   )}
...
export default function EventPopup() {
  ...
  const worldRecordImages = [
    "https://res.cloudinary.com/dvd7o5nph/image/upload/v1771673388/WORLD_RECORD_POSTER.jpg_kkmzi9.jpg", // From your uploaded image file name
    "https://res.cloudinary.com/dvd7o5nph/image/upload/v1771673357/WhatsApp_Image_2026-02-21_at_3.54.17_PM_ccvhn3.jpg"    // Update this to your second image's name
  ];

  const nextImage = () => {
    setCurrentImageIndex((prev) => (prev + 1) % worldRecordImages.length);
  };

  const prevImage = () => {
    setCurrentImageIndex((prev) => (prev - 1 + worldRecordImages.length) % worldRecordImages.length);
  };

  // --- NEW: AUTO-PLAY CAROUSEL EFFECT ---
  useEffect(() => {
    let interval: NodeJS.Timeout;
    
    // Only auto-play if the world record tab is active and there is more than 1 image
    if (activeTab === 'world-record' && worldRecordImages.length > 1) {
  ...
  }, [activeTab, worldRecordImages.length]);
          ...
          >
            ...
            <div className="w-full md:w-3/5 flex flex-col h-full bg-white">
              ...
              <div className="flex-grow overflow-y-auto p-6 bg-white relative">
                ...
                {/* === TAB 0: WORLD RECORD (NOW WITH CAROUSEL) === */}
                {activeTab === 'world-record' && (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    ...
                    className="h-full flex flex-col items-center space-y-4"
                  >
                    <div className="w-full h-full relative min-h-[300px] md:min-h-[400px] rounded-xl overflow-hidden bg-purple-50 flex items-center justify-center border border-purple-100 group">
                      ...
                      <AnimatePresence mode="wait">
                        ...
                        >
                          <Image 
                            src={worldRecordImages[currentImageIndex]} 
                            alt={`World Record Attempt Poster ${currentImageIndex + 1}`} 
                            fill
                            className="object-contain p-2" 
                          />
                        </motion.div>
                      ...
                      {worldRecordImages.length > 1 && (
                        <>
                          <button 
                            onClick={prevImage}
                            className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-white/90 hover:bg-white text-purple-700 rounded-full shadow-md opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10"
                          >
                            <ChevronLeft className="w-5 h-5" />
                          </button>
                          <button 
                            onClick={nextImage}
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-white/90 hover:bg-white text-purple-700 rounded-full shadow-md opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10"
                          >
                          ...
                          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10 bg-white/50 backdrop-blur-sm px-3 py-1.5 rounded-full">
                            {worldRecordImages.map((_, idx) => (
                              <button
                                key={idx}
                                onClick={() => setCurrentImageIndex(idx)}
                                className={`w-2 h-2 rounded-full transition-all ${
                                  idx === currentImageIndex ? 'bg-purple-700 w-4' : 'bg-purple-300 hover:bg-purple-500'
                                }`}
                              />
                            ))}
                          </div>
                  ...
                  {activeTab === 'world-record' && (
                    <div className="space-y-3">
                      <Link 
                        ...
                        className="w-full py-3 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition-all flex items-center justify-center gap-2 text-sm shadow-md group"
                      >
                        Click Here to Know More <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                      </Link>
                    </div>
                  )}
