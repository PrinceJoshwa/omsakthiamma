...
// export default function EventPopup() {
//   const [isOpen, setIsOpen] = useState(false);
//   const [activeTab, setActiveTab] = useState<'event' | 'train' | 'abhishekam'>('event');
//   const [searchTerm, setSearchTerm] = useState("");
//   const [daysToGo, setDaysToGo] = useState(0);
//   const [mainEvent, setMainEvent] = useState(masterEventList[0]);
...
//             <div className="w-full md:w-3/5 flex flex-col h-full bg-white">
//               <div className="flex-none flex border-b border-gray-200">
//                 <button onClick={() => setActiveTab('event')} className={`flex-1 py-3 md:py-5 text-[10px] md:text-xs font-bold uppercase tracking-wide transition-colors relative ${activeTab === 'event' ? 'text-[#a7150b] bg-red-50/30' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'}`}>
//                   <span className="flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2"><Calendar className="w-4 h-4" /> <span>Festival</span></span>
//                   {activeTab === 'event' && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 w-full h-1 bg-[#a7150b]" />}
//                 </button>
//                 <button onClick={() => setActiveTab('abhishekam')} className={`flex-1 py-3 md:py-5 text-[10px] md:text-xs font-bold uppercase tracking-wide transition-colors relative ${activeTab === 'abhishekam' ? 'text-amber-600 bg-amber-50/30' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'}`}>
//                   <span className="flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2"><Droplet className="w-4 h-4" /> <span>பால் அபிஷேகம்</span></span>
//                   {activeTab === 'abhishekam' && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 w-full h-1 bg-amber-500" />}
//                 </button>
//                 <button onClick={() => setActiveTab('train')} className={`flex-1 py-3 md:py-5 text-[10px] md:text-xs font-bold uppercase tracking-wide transition-colors relative ${activeTab === 'train' ? 'text-blue-600 bg-blue-50/30' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'}`}>
//                   <span className="flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2"><TrainFront className="w-4 h-4" /> <span>Trains</span></span>
//                   {activeTab === 'train' && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 w-full h-1 bg-blue-600" />}
//                 </button>
//               </div>
...
//                 {/* --- ABHISHEKAM TAB (NEW CONTENT) --- */}
//                 {activeTab === 'abhishekam' && (
//                   <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 h-full flex flex-col pb-4 font-tamil">
                    
//                     <div className="text-center space-y-4 pt-2">
//                       <h2 className="text-[#a7150b] font-bold text-xl md:text-2xl leading-snug">
//                         ஆடிப்பூரப் பால் அபிஷேகத்தை <br/> உங்கள் சார்பாகச் செய்யுங்கள்
//                       </h2>
//                       <div className="bg-amber-50 text-amber-900 p-4 rounded-xl border border-amber-200 inline-block shadow-sm">
//                         <p className="text-sm md:text-base font-bold italic">
//                           "பக்திக்கு தூரம் ஒரு தடையில்லை;<br className="md:hidden" /> பக்தியின் தூய்மையே அம்மாவே சேர்ந்திடும்!"
//                         </p>
//                       </div>
//                     </div>

//                     <div className="text-gray-700 text-sm leading-relaxed space-y-4 text-justify px-1">
//                       <p>
//                         அருள்மிகு ஆதிபராசக்தி சித்தர் பீடத்தில் ஆடிப்பூரப் பெருவிழா என்பது அபரிமிதமான ஆன்மீக சக்தி நிறைந்த காலமாகும். உங்கள் சொந்தக் கைகளால் பால் அபிஷேகம் செய்வது எல்லையில்லா மகிழ்ச்சியைத் தரும் என்றாலும், சில நேரங்களில் தூரம், உடல்நலம் அல்லது வேலைப்பளு காரணமாக நம்மால் நேரில் வர முடிவதில்லை என்பதை நாங்கள் அறிவோம். <strong>உண்மையான பக்தி என்பது நேரில் இருப்பதை விட, இதயத்தின் தூய்மையால் அளவிடப்படுகிறது என்பதே அம்மா நமக்குக் கற்பிக்கும் பாடம்.</strong>
//                       </p>
//                       <p>
//                         எந்த ஒரு பக்தரும் இந்தப் புனிதமான வாய்ப்பைத் தவறவிடக் கூடாது என்பதற்காக, உங்கள் சார்பாகப் பால் அபிஷேகம் செய்யும் வசதியை நாங்கள் ஏற்படுத்தியுள்ளோம். காணிக்கை செலுத்துவதன் மூலம், மதிப்பிற்குரிய திருமதி. அம்மா உங்கள் சார்பாகச் சுயம்புவுக்கு அபிஷேகம் செய்வார்கள். <strong>உங்களின் பிரத்யேகப் பிரார்த்தனைகள் நேரடியாகக் கருவறைக்குக் கொண்டு செல்லப்படும்.</strong>
//                       </p>
//                     </div>

//                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
//                        {/* Step 1 */}
//                        <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 flex gap-4 items-center hover:shadow-md transition-shadow">
//                           <div className="p-3 bg-purple-200 text-purple-700 rounded-full shrink-0"><Globe className="w-5 h-5"/></div>
//                           <div>
//                              <h4 className="font-bold text-purple-900 text-sm">பதிவு செய்க</h4>
//                              <p className="text-xs text-purple-800 mt-1">எங்களின் இணையதளத்திற்குச் செல்லவும்</p>
//                           </div>
//                        </div>
//                        {/* Step 2 */}
//                        <div className="bg-green-50 p-4 rounded-xl border border-green-100 flex gap-4 items-center hover:shadow-md transition-shadow">
//                           <div className="p-3 bg-green-200 text-green-700 rounded-full shrink-0"><FileText className="w-5 h-5"/></div>
//                           <div>
//                              <h4 className="font-bold text-green-900 text-sm">விவரங்கள்</h4>
//                              <p className="text-xs text-green-800 mt-1">உங்கள் பெயர், நட்சத்திரத்தை உள்ளிடவும்</p>
//                           </div>
//                        </div>
//                        {/* Step 3 */}
//                        <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 flex gap-4 items-center hover:shadow-md transition-shadow">
...
//                        </div>
//                        {/* Step 4 */}
//                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex gap-4 items-center hover:shadow-md transition-shadow">
//                           <div className="p-3 bg-blue-200 text-blue-700 rounded-full shrink-0"><Sun className="w-5 h-5"/></div>
//                           <div>
//                              <h4 className="font-bold text-blue-900 text-sm">ஆசிர்வாதம்</h4>
//                              <p className="text-xs text-blue-800 mt-1">திருமதி அம்மா காணிக்கையை நிறைவேற்றுவார்கள்</p>
//                           </div>
//                        </div>
//                     </div>

//                     <div className="bg-gradient-to-r from-[#a7150b] to-[#8a0d08] p-5 rounded-xl text-center shadow-lg mt-4 text-white">
//                       <p className="font-bold text-lg mb-1">பக்தியால் தூரத்தைக் கடப்போம்.</p>
//                       <p className="text-sm text-white/90">அம்மாவின் அருள் உங்கள் வாழ்வில் நிறையட்டும்.</p>
//                       <p className="text-xs mt-3 opacity-75">- மேல்மருவத்தூர் ஆதிபராசக்தி ஆன்மீக இயக்கம்</p>
//                     </div>
...
export default function EventPopup() {
  const [isOpen, setIsOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<'event' | 'train' | 'abhishekam'>('abhishekam');
  const [searchTerm, setSearchTerm] = useState("");
  const [daysToGo, setDaysToGo] = useState(0);
  const [mainEvent, setMainEvent] = useState(masterEventList[0]);
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4 py-4 md:py-8">
          ...
          >
            ...
            <div className="w-full md:w-3/5 flex flex-col h-full bg-white">
              ...
              {/* REORDERED TABS: 1. Abhishekam, 2. Festival, 3. Trains */}
              <div className="flex-none flex border-b border-gray-200">
                <button onClick={() => setActiveTab('abhishekam')} className={`flex-1 py-3 md:py-5 text-[10px] md:text-xs font-bold uppercase tracking-wide transition-colors relative ${activeTab === 'abhishekam' ? 'text-amber-600 bg-amber-50/30' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'}`}>
                  <span className="flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2"><Droplet className="w-4 h-4" /> <span>பால் அபிஷேகம்</span></span>
                  {activeTab === 'abhishekam' && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 w-full h-1 bg-amber-500" />}
